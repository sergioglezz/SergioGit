...
class MultipleController extends AbstractController
{
    ...
    public function show(MultipleRepository $multipleRepository, int $id): Response
    {
        return $this->render('multiple/show.html.twig', [
            'multiple' => $multipleRepository->findOneBy(['id' => $id]),
        ]);
    }

    #[Route('/{id}/edit', name: 'app_multiple_edit', methods: ['GET', 'POST'])]
    public function edit(Request $request, MultipleRepository $multipleRepository, int $id): Response
    {
        $multiple = $multipleRepository->findOneBy(['id' => $id]);
        $form = $this->createForm(MultipleType::class, $multiple);
        $form->handleRequest($request);
    ...
    public function delete(Request $request, MultipleRepository $multipleRepository, int $id): Response
    {
        $multiple = $multipleRepository->findOneBy(['id' => $id]);
